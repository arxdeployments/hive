...
class AccessPartyType(str, enum.Enum):
    """What a rule's two sides refer to. Deliberately not a FK — one column has
    to point at either users or departments, so referential integrity is traded
    for a single evaluable table. Ids are UUIDs and never reused, so a rule left
    behind by a deleted user or department simply stops matching; access.py
    tolerates that rather than depending on cascade cleanup."""
...
class ChatAccessRule(Base):
    """One SYMMETRIC "these two may talk" (or may not) rule.

    Reachability is DENY BY DEFAULT: a pair with no matching rule cannot start a
    conversation. Rules exist at two granularities and MOST SPECIFIC WINS:

        3  user     <-> user           an individual exception
        2  user     <-> department     one person against a whole department
        1  department <-> department   the broad policy

    At EQUAL specificity, DENY WINS. That only arises at level 2, where both
    "A <-> dept(B)" and "B <-> dept(A)" can exist and disagree; resolving that
    towards the permissive side would let one department's admin quietly widen
    another's restriction.

    Symmetric, not directional: "who can chat with who" has no natural sender.
    The pair is therefore stored CANONICALLY — access.canonical_pair() sorts the
    two sides — so an unordered pair has exactly one row and the unique
    constraint below actually prevents duplicates.
    """

    __tablename__ = "chat_access_rules"
    __table_args__ = (
        UniqueConstraint("a_type", "a_id", "b_type", "b_id", name="uq_chat_access_rule_pair"),
        Index("ix_chat_access_rules_org", "org_id"),
    )

    id: Mapped[uuid.UUID] = _uuid_pk()
    org_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("organizations.id", ondelete="CASCADE"))
    a_type: Mapped[AccessPartyType] = mapped_column(
        Enum(AccessPartyType, name="access_party_type", values_callable=lambda e: [m.value for m in e])
    )
    a_id: Mapped[uuid.UUID]
    b_type: Mapped[AccessPartyType] = mapped_column(
        Enum(AccessPartyType, name="access_party_type", values_callable=lambda e: [m.value for m in e])
    )
    b_id: Mapped[uuid.UUID]
...
class SendPolicy(Base):
    """What KINDS of thing a user (or a whole department) may send.

    Separate from ChatAccessRule on purpose: reachability is about a PAIR, this
    is about one sender regardless of who they are talking to.

    Unlike reachability this is ALLOW by default — absent a row, everything is
    permitted, which is exactly today's behaviour. That matches the requirement
    that the control can be toggled on and off: creating a row turns it on for
    that scope, deleting the row turns it off.

    Most specific wins here too, but as a WHOLE ROW rather than field by field:
    a user-scoped policy replaces their department's outright. Merging the two
    per-field would make "this one nurse may also send video" impossible to
    express without silently re-granting whatever the department later loosens.

    doc_extensions is NULL for "every document type storage.classify accepts",
    or a list like [".pdf", ".xlsx"] for a whitelist. It only ever NARROWS —
    access.py intersects it with DOC_EXTS, so an admin cannot whitelist ".exe"
    into existence.
    """

    __tablename__ = "send_policies"
    __table_args__ = (UniqueConstraint("scope_type", "scope_id", name="uq_send_policy_scope"),)

    id: Mapped[uuid.UUID] = _uuid_pk()
    org_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("organizations.id", ondelete="CASCADE"))
    scope_type: Mapped[AccessPartyType] = mapped_column(
        Enum(AccessPartyType, name="access_party_type", values_callable=lambda e: [m.value for m in e])
    )
    scope_id: Mapped[uuid.UUID]

    allow_text: Mapped[bool] = mapped_column(default=True)
    allow_image: Mapped[bool] = mapped_column(default=True)
    allow_video: Mapped[bool] = mapped_column(default=True)
    allow_audio: Mapped[bool] = mapped_column(default=True)
    allow_document: Mapped[bool] = mapped_column(default=True)
    # NULL = all document types; list[str] of lowercase dotted extensions = whitelist.
    doc_extensions: Mapped[dict | None] = mapped_column(JSONB, default=None)
...
class AdminDepartment(Base):
    """Which departments an org admin actually administers.

    Net-new: before this, org_admin was scoped only by users.org_id, so every
    admin reached their whole organization and there was no way to express
    "manages Radiology and Imaging". Many-to-many because one manager covering
    two departments is the ordinary case, and the alternative (a second account)
    is worse.

    An admin with NO rows here is org-wide, which is what every existing
    org_admin is migrated to — otherwise the migration would silently strip
    every current admin of their reach.
    """
