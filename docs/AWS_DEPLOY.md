...
| EBS root volume | 30 GB gp3 | ~$2.50 |
...
| **Private repo + deploy key** | Generate an SSH key, add the public half as a read-only deploy key on the repo, put the private half in SSM SecureString; `user_data` fetches it, writes `~/.ssh/id_ed25519`, clones over SSH. | Default recommendation. One extra secret to manage, zero exposure. |
...
7. **Decisions to write into `terraform.tfvars`:** the domain name, the ACME contact
   email (Let's Encrypt expiry notices), the superadmin email, the AWS region, and
   the git clone URL.
...
superadmin_email   = "admin@rhythmrx.ai"
```

Do **not** put secrets in this file. Terraform generates the JWT signing key, the
LiveKit API secret, the database password and the initial superadmin password with
`random_password`, and stores them as SSM SecureString parameters under
`/rxhive/<environment>/`. The instance reads them at boot with its instance role and
renders `/opt/rxhive/infra/.env` (root-owned, mode 0600).
...
| `rds_endpoint` | DB host, for manual psql/snapshot work |
| `s3_bucket` | attachment bucket name |
| `ssm_parameter_prefix` | e.g. `/rxhive/prod` — where the secrets live |

### 3.5 Watch the first boot

The instance clones the repo, renders `.env` from SSM, and runs
`docker compose -f infra/docker-compose.prod.yml up -d --build`. The image build takes
5–10 minutes on a t3.large; the API is not reachable before it completes.

```bash
aws ssm start-session --target $(terraform output -raw instance_id)

# on the instance:
sudo tail -f /var/log/cloud-init-output.log        # provisioning, incl. the build
cd /opt/rxhive
...
and then `[seed] superadmin created: …`.
...
need to iterate, point Caddy at the LE staging CA first (`acme_ca
https://acme-staging-v02.api.letsencrypt.org/directory` in the Caddyfile), get a
successful staging issuance, then switch back.

---

## 4. First login

The superadmin is seeded on the API's first boot from
`RXHIVE_SEED_SUPERADMIN_EMAIL` / `RXHIVE_SEED_SUPERADMIN_PASSWORD`.

- **Email:** whatever you set as `superadmin_email` (default `admin@rhythmrx.ai`).
- **Password:** generated by Terraform, stored in SSM SecureString. Read it:

```bash
aws ssm get-parameter \
  --name "$(terraform output -raw ssm_parameter_prefix)/seed_superadmin_password" \
  --with-decryption --query 'Parameter.Value' --output text
```

Sign in at `https://chat.example.com` and **change the password in-app immediately**
(profile menu → Settings → Change password), then create your organizations,
departments and users from the admin portal.

Two things worth knowing about the seed:

- It is **idempotent and non-destructive**: it creates the superadmin only if that
  email does not already exist. Rotating the password in-app is permanent — a redeploy
  will not reset it back to the SSM value.
- The SSM parameter is therefore only a *bootstrap* credential. After you have rotated
  it in-app, the parameter value is stale; treat it as such.
...
(`/ws` through Caddy) and Redis pub/sub are both working. If it only appears after a
reload, the WebSocket is not connecting.
...
check 1. Compose restarts under `restart: unless-stopped`, and the Docker service is
enabled at boot — if the stack does not come back by itself, fix that now rather than
...
There is no SSH. If Session Manager cannot connect, the instance has lost outbound
access to the SSM endpoints or the SSM agent is down — check the instance status
checks in the console first.
...
sudo tail -100 /var/log/cloud-init-output.log                    # first-boot provisioning
```
...
node state — all reconstructible. Users flicker offline for a moment. Restarting
...
- **Point-in-time recovery**: anywhere within the retention window, to ~5-minute
  ...
  ```
  Then point `RXHIVE_DATABASE_URL` at the new endpoint (update the SSM parameter and
  restart `api`), verify, and retire the old instance.
- **Automated backups are deleted when the instance is deleted** unless you take a
  final snapshot. Keep `deletion_protection` and `skip_final_snapshot = false` on.
- **S3**: attachments live only in S3. Enable bucket versioning (and a lifecycle rule
  to expire noncurrent versions) if you want protection from accidental deletes; add
  cross-region replication only if your RPO demands it.
...
```bash
aws ssm put-parameter --name "/rxhive/prod/<name>" --type SecureString \
  --value "<new value>" --overwrite
# then, on the instance:
sudo /opt/rxhive/scripts/render-env.sh   # re-renders /opt/rxhive/infra/.env from SSM
sudo docker compose -f infra/docker-compose.prod.yml up -d
```

Rotating `RXHIVE_SECRET_KEY` invalidates every issued JWT — all users are logged out.
Rotating `LIVEKIT_API_SECRET` requires restarting `livekit` and `api` together, or
token signing and verification disagree and every call fails.
...
`terraform apply`. Terraform stops, resizes and starts the instance; the Elastic IP and
all data survive. Raise `API_WORKERS` at the same time (rule of thumb: 2× vCPU).
Downtime is a couple of minutes. This buys a lot and costs an afternoon.
...
work identically across multiple workers and processes. So:
   1. Move Redis to **ElastiCache** (single node is fine to start) and point
      `RXHIVE_REDIS_URL` at it. LiveKit uses the same Redis — update `infra/livekit.yaml`.
...
instance with its own Elastic IP and the same UDP/TCP ports open, then set
`RXHIVE_LIVEKIT_URL` (browser-facing, `wss://livekit.example.com`) and
`RXHIVE_LIVEKIT_HEALTH_URL` (server-facing, for the `/api/health` probe) accordingly.
For multi-node LiveKit, every node must share one Redis — which you already did in
step 2.
...
- **No SSH.** Port 22 is not open in any security group and no key pair is attached.
  Administrative access is **SSM Session Manager** only, via the instance's IAM role
  (`AmazonSSMManagedInstanceCore`) — so access is granted and revoked with IAM, every
  session is logged in CloudTrail, and there is no key material to leak or rotate.
...
- **The S3 bucket blocks all public access** (all four block-public-access flags on),
  has no bucket policy granting anonymous reads, and is encrypted server-side.
  Attachments are never served directly: a request goes to `/api/media/<id>`, which
  checks that the caller is a member of the conversation, and only then redirects to a
  **presigned URL valid for 300 seconds**. A leaked URL expires in five minutes; a
  leaked object key on its own is useless.
- **Secrets live in SSM Parameter Store as SecureString**, encrypted with KMS and
  ...
  committed to git** — `.env` and `*.pem` are gitignored, and `terraform.tfvars`
  contains no secret values. The one thing that *does* contain secrets is the Terraform
  state file: keep it in an encrypted S3 backend.
- **The API refuses to boot with a weak configuration.** In production it validates
  `RXHIVE_SECRET_KEY`, `RXHIVE_LIVEKIT_API_SECRET` and `RXHIVE_S3_SECRET_KEY` against a
  list of known placeholder values and a 32-character minimum, and raises at startup if
  any fails. This is deliberate: it makes "we shipped the demo signing key" impossible
  rather than merely unlikely.
- **Auth is same-origin cookies, not bearer tokens.** Sessions are httpOnly + Secure +
  SameSite cookies (`RXHIVE_COOKIE_SECURE=true`), and every mutating request must carry
...
- **S3 credentials.** The app uses a scoped IAM access key (bucket-limited:
  `GetObject`/`PutObject`/`DeleteObject` on `arn:…:<bucket>/*` plus `ListBucket` on the
  bucket), stored in SSM. It does *not* use the EC2 instance role for S3, even though
  `backend/app/services/storage.py` supports that path — the production config
  validator rejects an empty `RXHIVE_S3_SECRET_KEY` as a placeholder, so the app cannot
  boot without static keys. (AWS secret access keys are 40 characters, comfortably over
  the 32-character minimum.) If you want instance-role credentials, that validator must
  learn to allow a deliberately-empty S3 key first.
...
| **Calls connect but there is no audio or video** (black tiles, silence, one side or both) | UDP `50000-50100` blocked, or LiveKit advertising a private IP | Check the SG has `udp 50000-50100` from `0.0.0.0/0`; check `docker compose logs livekit \| grep -i external` prints the **Elastic IP**, not `10.x.x.x` — `rtc.use_external_ip: true` in `infra/livekit.yaml` is what makes ICE candidates routable from behind NAT. Also confirm `7881/tcp` is open for clients on UDP-hostile networks. A corporate firewall on the *client* side blocking UDP produces the same symptom for that user only. |
| **Call fails immediately; `/api/health` shows `"livekit":"unreachable"`, `calls_available:false`** | SFU container down, or `RXHIVE_LIVEKIT_HEALTH_URL` wrong | `docker compose ps livekit`; restart it. The health URL must be the *server-side* address `http://livekit:7880` — the browser-facing `/livekit` path is not resolvable from inside the API process. |
| **Everyone can call but tokens are rejected** | `LIVEKIT_API_KEY`/`_SECRET` differ between the `api` and `livekit` containers | They come from the same `.env`; if you rotated one, restart **both** services together. |
| **502 from Caddy** | The upstream container is down or still starting — most often `api` exited because a migration failed or config validation raised | `docker compose ps` (look for `Exited`), then `docker compose logs api --tail 100`. During a `--build` deploy a 502 for 30–90s is expected. |
| **DB connection refused / timeout**; health shows `"database":"disconnected"` | RDS not reachable from the instance, or the URL is wrong | From the instance: `nc -zv <rds_endpoint> 5432`. If it hangs, the DB security group is not allowing the app SG on 5432. Check `RXHIVE_DATABASE_URL` uses the `postgresql+asyncpg://` scheme and the RDS endpoint hostname (not `postgres`, the dev container name). If the error mentions SSL or "no encryption", append `?ssl=require` — RDS PostgreSQL 15+ sets `rds.force_ssl=1` by default. |
...
| **API refuses to boot** — logs show `RXHIVE_SECRET_KEY is a known placeholder` or `must be at least 32 characters` | A placeholder or short secret reached production | Intentional guard. Set a real value: `openssl rand -hex 32`, write it to the SSM parameter, re-render `.env`, restart `api`. Applies to `RXHIVE_SECRET_KEY`, `RXHIVE_LIVEKIT_API_SECRET` and `RXHIVE_S3_SECRET_KEY` — and note that an **empty** value counts as a placeholder. |
| **Password/URL parse errors on boot** | The generated DB password contains characters that are special in a URL (`@`, `/`, `#`, `:`) | Keep `random_password` restricted to alphanumerics, or percent-encode the password inside `RXHIVE_DATABASE_URL`. |
| **Images/attachments do not load** — broken thumbnails; console shows `Refused to load the image … Content Security Policy` | `frontend/nginx.conf` sets `img-src 'self' data: blob:`. `/api/media/<id>` redirects to a presigned S3 URL; after the redirect the browser is loading a **cross-origin** image, which the CSP blocks. | Two options. **(a) Same-origin proxy:** keep the browser on your domain by setting `RXHIVE_S3_PUBLIC_ENDPOINT=/s3` and having Caddy proxy `/s3/*` to the bucket — you must rewrite the upstream `Host` header to the S3 host, because SigV4 signs `Host` and passing your domain through yields `SignatureDoesNotMatch`. **(b) Direct to S3:** set `RXHIVE_S3_PUBLIC_ENDPOINT` to the bucket origin and add that exact origin to `img-src`, `media-src` and `connect-src` in `frontend/nginx.conf`, then rebuild the web image. Verify what the presigned URLs actually look like before choosing: `docker compose exec api python -c "import asyncio; from app.services.storage import presign_get; print(asyncio.run(presign_get('probe.jpg')))"` — note the S3 client uses virtual-hosted-style URLs (`https://<bucket>.s3.<region>.amazonaws.com/…`) for AWS endpoints, which the endpoint-rewrite in `presign_get` will not match. |
| **Attachment *download* fails** (thumbnail renders, download button does not) | The download path uses `fetch()`, so the cross-origin redirect needs S3 CORS as well as CSP | Add a CORS rule to the bucket: `AllowedOrigins: ["https://chat.example.com"]`, `AllowedMethods: ["GET","HEAD"]`, `AllowedHeaders: ["*"]`. Not needed at all if you use the same-origin `/s3` proxy above. |
| **Upload returns 403** | The IAM key is missing a permission, or the bucket name in `RXHIVE_S3_BUCKET` does not match the Terraform-created bucket | Compare with `terraform output -raw s3_bucket`. The policy needs `s3:PutObject`, `s3:GetObject`, `s3:DeleteObject` on `<bucket>/*` and `s3:ListBucket` on `<bucket>`. |
| **Messages only appear after a page refresh** | The WebSocket is not connecting through Caddy | Check the browser network tab for `/ws` (status 101). Confirm the `handle /ws*` route exists in the production Caddyfile and that `redis` is healthy — fan-out between workers goes through Redis pub/sub. |
...
|---|---|---|
| `RXHIVE_ENVIRONMENT` | `production` | compose |
| `RXHIVE_DATABASE_URL` | `postgresql+asyncpg://<master>:<pw>@<rds_endpoint>:5432/rxhive` | SSM (password), Terraform (host) |
| `RXHIVE_REDIS_URL` | `redis://redis:6379/0` | compose (container on the same box) |
| `RXHIVE_SECRET_KEY` | 64-hex random | SSM SecureString |
| `RXHIVE_COOKIE_SECURE` | `true` | compose |
| `RXHIVE_TRUST_PROXY` | `true` (Caddy is the only ingress) | compose |
| `RXHIVE_CORS_ORIGINS` | *empty* — single origin, do not set | compose |
| `RXHIVE_S3_ENDPOINT` / `_BUCKET` / `_REGION` | real S3 | Terraform outputs |
| `RXHIVE_S3_ACCESS_KEY` / `_SECRET_KEY` | scoped IAM user key | SSM SecureString |
| `RXHIVE_S3_PUBLIC_ENDPOINT` | see the CSP row in §9 | compose |
| `RXHIVE_LIVEKIT_URL` | `/livekit` (browser resolves against the site origin) | compose |
| `RXHIVE_LIVEKIT_HEALTH_URL` | `http://livekit:7880` (server-side probe) | compose |
| `RXHIVE_LIVEKIT_API_KEY` / `_API_SECRET` | shared with the SFU | SSM SecureString |
| `RXHIVE_SEED_SUPERADMIN_EMAIL` / `_PASSWORD` | bootstrap admin | tfvars / SSM SecureString |
| `SITE_ADDRESS` | your domain — this is what makes Caddy do ACME | tfvars |
